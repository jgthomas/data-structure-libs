...
void testMERGE_SORT(void)
{
        ...
        for (int i = 0; i < NUM_TESTS; i++)
        {
                ...
                merge_sort(tests[i]->test, tests[i]->elem_size, 0, (tests[i]->data_size / tests[i]->elem_size), tests[i]->compare);
...
int main(void)
{
        ...
        // add tests
        if (NULL == CU_add_test(suite, "Merge sort", testMERGE_SORT))
        {
                CU_cleanup_registry();
                return CU_get_error();
        }
