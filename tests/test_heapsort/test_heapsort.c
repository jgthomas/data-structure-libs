...
/**
 * Override comparison function as heapsort uses a max-heap
 *
 * */
void change_comparison(TestCase **tests, int num_tests)
{
...
void testODD_NO_ELEMENTS(void)
{
        ...
        change_comparison(tests, NUM_TESTS);
...
int main(void)
{
        ...
        // add suite
        suite = CU_add_suite("Heapsort", 0, 0);
        if (NULL == suite)
        {
                CU_cleanup_registry();
                return CU_get_error();
