...
void testDL_LIST_FIND_AND_MOVE(void)
{
        TestCase **tests = make_tests(NUM_TESTS);
        ...
        for (int i = 0; i < NUM_TESTS; i++)
        {
                ...
                if (i == 0)
                {
                        ...
                        void *not_in_list_int = &n2;
                        tests[i]->answer = new_answer_int;
                        CU_ASSERT_FALSE(DL_list_find_and_move(&head, not_in_list_int, tests[i]->equal));
                        CU_ASSERT_TRUE(DL_list_find_and_move(&head, in_list_int, tests[i]->equal));
                        CU_ASSERT_TRUE(dll_array_match(head, tests[i]->answer, tests[i]->data_size, tests[i]->elem_size, tests[i]->equal));
                ...
                else if (i == 1)
                {
                        ...
                        void *not_in_list_char = &c2;
                        tests[i]->answer = new_answer_char;
                        CU_ASSERT_FALSE(DL_list_find_and_move(&head, not_in_list_char, tests[i]->equal));
                        CU_ASSERT_TRUE(DL_list_find_and_move(&head, in_list_char, tests[i]->equal));
                        CU_ASSERT_TRUE(dll_array_match(head, tests[i]->answer, tests[i]->data_size, tests[i]->elem_size, tests[i]->equal));
                }
                else if (i == 2)
                {
                        ...
                        void *not_in_list_string = &s2;
                        tests[i]->answer = new_answer_string;
                        CU_ASSERT_FALSE(DL_list_find_and_move(&head, not_in_list_string, tests[i]->equal));
                        CU_ASSERT_TRUE(DL_list_find_and_move(&head, in_list_string, tests[i]->equal));
                        CU_ASSERT_TRUE(dll_array_match(head, tests[i]->answer, tests[i]->data_size, tests[i]->elem_size, tests[i]->equal));
                }
...
int main(void)
{
        ...
        if (NULL == CU_add_test(suite, "Pushing to list", testDL_LIST_PUSH) ||
            ...
            NULL == CU_add_test(suite, "Find element and move to front of list", testDL_LIST_FIND_AND_MOVE))
        {
                CU_cleanup_registry();
                return CU_get_error();
        }
